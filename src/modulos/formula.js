...
const hexadecimalBinario = (valor) => {
    ...
    for (let m = 0; m < separador.length; m++) {
        let guardar = new Array(),
        resultado = new Array();

        /*Validar Datos hexadecimales*/
        for (let i = 0; i <separador[m].length; i++) {
            if(separador[m][i] == 'A' || separador[m][i] == 'a') guardar[i] = '10';
            else if(separador[m][i] == 'B' || separador[m][i] == 'b')guardar[i] = '11';
            else if(separador[m][i] == 'C' || separador[m][i] == 'c')guardar[i] = '12';
            else if(separador[m][i] == 'D' || separador[m][i] == 'd')guardar[i] = '13';
            else if(separador[m][i] == 'E' || separador[m][i] == 'e')guardar[i] = '14';
            else if(separador[m][i] == 'F' || separador[m][i] == 'f')guardar[i] = '15';
            else guardar[i] =separador[m][i];  
        }
...
const binarioHexadecimal = (valor) => {
    ...
    for (let m = 0; m < separador.length ; m++) {
        ...
        for(let i = separador[m].length-1 ; i >= 0 ; i--) almacenar[contadorAlmacen++] = separador[m][i];
